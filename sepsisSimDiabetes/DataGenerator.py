...
class DataGenerator(object):
    ...
    def simulate(
        ...
        if output_state_idx_type == "obs":
            emp_tx_mat = np.zeros(
                (Action.NUM_ACTIONS_TOTAL, State.NUM_OBS_STATES, State.NUM_OBS_STATES)
            )
            emp_r_mat = np.zeros(
                (Action.NUM_ACTIONS_TOTAL, State.NUM_OBS_STATES, State.NUM_OBS_STATES)
            )
            ...
            emp_tx_mat = np.zeros(
                (Action.NUM_ACTIONS_TOTAL, State.NUM_FULL_STATES, State.NUM_FULL_STATES)
            )
            emp_r_mat = np.zeros(
                (Action.NUM_ACTIONS_TOTAL, State.NUM_FULL_STATES, State.NUM_FULL_STATES)
            )
        ...
        for itr in tqdm(range(num_iters), disable=not (use_tqdm), desc=tqdm_desc):
            ...
            for step in range(max_num_steps):
                ...
                emp_tx_mat[this_action_idx, this_from_state_idx, this_to_state_idx] += 1
                emp_r_mat[
                    this_action_idx, this_from_state_idx, this_to_state_idx
                ] += step_reward
